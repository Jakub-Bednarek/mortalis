...
template <typename T> class UpgradesPoolFactory;

template <typename T> class MORTALIS_API UpgradesPool
{
...
public:
    ...
    void RegisterChain(Chain<T> &&Value)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("Upgrades chain with index: %d is already registered."),
    ...
    TArray<FUpgradeUIData> GetUIDataFromRandomlySelected(int32 Count)
    {
        ...
        for (auto i = 0; i < Count; i++)
        {
            ...
            SelectedUpgradesUIData.Add(
                RegisteredChains[NotSelectedChains[RandomElementIndex]].GetNextCandidate()->GetUIData());
    ...
    T SelecteNextUpgrade(const FUpgradeIndex UpgradeIndex)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("Lookup failed, no ChainIndex associated with given UpgradeIndex: %d."),
                   UpgradeIndex.Value);
            return nullptr;
        }
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("Lookup failed, no Chain associated with given ChainIndex: %d."),
                   ChainIndex->Value);
            return nullptr;
        }
